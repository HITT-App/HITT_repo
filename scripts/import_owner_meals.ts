...
const dir = args.find(
  (a, idx) => !a.startsWith("--") && !(emitSqlIdx >= 0 && idx === emitSqlIdx + 1),
);
...
function renderSql(recipes: ParsedRecipe[]): string {
  ...
  out.push(`-- Seeds the owner-curated meal library (source='owner') for these`);
  out.push(`-- categories: ${categories.join(", ")}.`);
  out.push("-- Idempotent: re-running this seed replaces only these categories,");
  out.push("-- leaving other owner-category rows untouched.");
  out.push("");
  out.push("BEGIN;");
  out.push("");
  out.push("-- Cascade deletes ingredients + steps via FK ON DELETE CASCADE.");
  out.push(
    "DELETE FROM public.recipes WHERE source = 'owner' AND category IN (" +
